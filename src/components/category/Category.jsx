...
export default function Category({ className }) {
  ...
  useEffect(() => {
    setCat(param.id.substring(1, param.id.length));
  }, [param]);
  const data = useCategory({ con: limits, category: cat });
  return data.length === 0 ? (
    ...
    <>
      <div
        className={` ${className} grid gap-6 sm:grid-cols-2 md:grid-cols-4 pt-10`}
      >
        {data.map((item, index) => (
          <motion.div
